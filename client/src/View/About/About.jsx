  ...
  return (
        ...
        <h1>Conócenos</h1>
         ...
         {discipulosHenry.map((estudiante) => (
           <Col key={estudiante.id}>
              <Card  style={{ width: '14rem', height: '16rem', alignItems:'center', background:'dark'}} className={style.customCard} >
              <Card.Img variant="top" src={estudiante.foto} style={{ width: "70%", alignSelf: "center" }}className={style.img} />
              <Card.Body style={{ width: "98%" }}> 
                <Card.Title style={{ fontSize: "1.10rem" }}>{estudiante.name}</Card.Title>
              </Card.Body>
                <Button variant="primary" onClick={()=>handleShow (estudiante.id)} className={style.button} >Contáctame </Button>
              </Card>
            </Col>
          ))}
    ...
    <Modal
      ...
      >
          ...
          <div>
            <Modal.Body>
              <img src={estudian[0]?.foto} alt={estudian[0]?.name} />
            </Modal.Body>
            <Modal.Body>
              <h6>COHORTE: {estudian[0]?.cohorte}</h6>
              <h6>EMAIL: <a href={`mailto:${estudian[0]?.email}`}>{estudian[0]?.email}</a></h6> 
              <h6>GIT-HUB: {estudian[0]?.gitHub} </h6>
              <h6>PORTFOLIO: {estudian[0]?.portfolio} </h6> 
            </Modal.Body>
          </div>
          <Modal.Footer>
